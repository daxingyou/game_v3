...
function KNBtn:new(path, file, x, y, params, group)
	...
	this.layer:setContentSize(cc.size(this.item[1]:getContentSize().width,this.item[1]:getContentSize().height))
...
--				local text = display.strokeLabel(v[1], 0, 0, v[2] or 16, cc.c3b( 0x2c , 0x00 , 0x00 ) , 2 , cc.c3b( 0x40 , 0x1d , 0x0c ))
				local text = display.strokeLabel(v[1], 0, 0, v[2] or 16, v[3] or cc.c3b( 0x2c , 0x00 , 0x00 ) )
...
--			local text = CCLabelTTF:create(this.params["text"][1],FONT,this.params["text"][2])
			local text = display.strokeLabel(this.params["text"][1], 0, 0, this.params["text"][2] or 16, this.params["text"][3] or cc.c3b( 0x2c , 0x00 , 0x00 ) )
			table.insert(this.textItems, text)
...
--				audio.playSound("sound/click.mp3")
				...
				if cc.rectContainsPoint(this:getRange(),cc.p(x,y)) then
					...
					moveOn = true
--					lastX = x
					lastX = cc.rectGetMinX(this:getRange())
					lastY = cc.rectGetMinY(this:getRange())
	...
	this.layer:addNodeEventListener(cc.NODE_TOUCH_CAPTURE_EVENT,function(event)
		 	return this.layer:onTouch(event.name,event.x,event.y)
		 end)
