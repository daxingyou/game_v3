...
--		self._index=self
		...
		local KNMask = require("common.UI.KNMask")--require("GameLuaScript/Common/KNMask")
		local mask = nil
...
--			end
		...
		local function createLayout(type , args)
			...
			if type == 0 then
				...
				content =  display.newSprite("common/tip_bg.png")
				...
				if isConfirm then
					-- 使用这种控件才能点击到
					local textures = {"scene/login/select_btn.png","",""}
					confirmBtn = ccui.Button:create()
					 	confirmBtn:loadTextures(textures[1],textures[2],textures[3])
					 	confirmBtn:setTitleText("确定")
					 	confirmBtn:setTitleColor(cc.c3b(0, 255, 0))
					 	confirmBtn:setTitleFontSize(30)
					 	-- confirmBtn:setPosition(display.cx, display.cy)
	            		confirmBtn:addTouchEventListener(function(widget, eventType)
		                    if eventType == 2 then -- ended
		                        args.confirmFun()
								backFun()
		                    end
		                end)
					...
					-- confirmBtn = KNBtn:new("common/" , {"btn_bg_red.png"} , 0 , 0 , {
					-- 	front = args.confirmText or "common/confirm.png" ,
				...
				--如果有取消时按钮
				if isCancel then
					cancelBtn = KNBtn:new("common/" , {"btn_bg_red.png"} , 0 , 0 , {
						front = args.cancelText or "common/cancel.png",
						scale = true,
						priority = args.priority or  -500,
						callback = function()
							args.cancelFun()
				...
				if isConfirm and isCancel then--如果两个按钮同时存在的坐标设置
					--重新设置确定按钮坐标
					setAnchPos(confirmBtn , display.cx/2 - 208 , 35)
					setAnchPos(cancelBtn , display.cx/2 + 5 , 35)
				else
					if isConfirm then--如果只有确认按钮时的坐标
						setAnchPos(confirmBtn , confirmBtn:getContentSize().width/2 , 35)
					elseif isCancel then--如果只有取消按钮时的坐标
						setAnchPos(cancelBtn , display.cx/2 - 102 , 35)
					end
				end
					...
					onComplete = function()
						transition.moveBy(target , {
							...
							onComplete = function()
								...
								-- target:removeFromParent(true)	-- 清除自己
