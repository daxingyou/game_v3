...
local KNMask = require("common.UI.KNMask")
...
function M:new(args)
	...
	local loading_bg = display.newSprite("loading/loading_bg.png")
	display.align(loading_bg , display.CENTER , display.cx , display.cy)
	group:addChild(loading_bg)

	local loading_sprite = display.newSprite("loading/loading.png")
	...
	local text = CCLabelTTF:create(args.text or "正在加载，请稍等.." , FONT , 22)
	display.align(text , display.CENTER , display.cx + 35 , display.cy)
	text:setColor( cc.c3b( 0xff , 0xff , 0xff ) )
	group:addChild(text)

	this.view = KNMask:new({item = group , click = click , priority = -140})
