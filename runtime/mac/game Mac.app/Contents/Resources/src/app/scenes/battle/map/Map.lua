...
function Map:onExit()
    ...
    -- issue : 不删除下次进入场景将导致CCSprite is not using the same texture id
    display.removeSpriteFramesWithFile("scene/battle/buff.plist","scene/battle/buff.png")

    if self.bgLayer_ then
        self.bgLayer_:removeFromParent(true)
        self.bgLayer_ = nil
    end
end
------------------------------------------------------------------------------
        ...
        for y=1,self:getDMap():getMapSize().height do
            for x=1,self:getDMap():getMapSize().width do
               local cell = self:getDMap():getMapCell(x-1, y-1)
                --local pos_ =
                    local rect = display.newRect(cell.rect,{fillColor = cc.c4f(1.0, 1.0, 1.0, 0)})
...
function Map:createView(parent)
    ...
    display.addSpriteFrames("scene/battle/buff.plist","scene/battle/buff.png")
    ...
    -- buff
    if self.batch_buff == nil then
        self.batch_buff = display.newBatchNode("scene/battle/buff.png",128):addTo(parent,MapConstants.MAP_Z_1_0)
    end
...
function Map:spawnSelf(parent)
    local formation_mgr =PLAYER:get_mgr("formations")
    self.battle_info[MapConstants.PLAYER_CAMP]=formation_mgr.info
    -- 1. 显示我方(左边)
    ...
    for key , v in pairs(Fdata_) do
        ...
        if info.index>0 and info.GUID > 0 then --需要有值
            ...
            -- 创建武将，从玩家武将数据得到
            local data = PLAYER:get_mgr("hero"):get_info(info.GUID)
...
function Map:spawnEnemy(parent)
    ...
    self.battle_info[MapConstants.ENEMY_CAMP]={fId=mapMonster.Fid,level=0}
    local count = 0
    self.bulidData = nil
    Formation:build( fid, function ( param, index)
        ...
        -- 创建武将，数据从表里面生成
        local data = configMgr:getConfig("hero"):GetHeroDataById(monsterid)
...
function Map:scanByTargetConditions(options)
    local target_count = 0
    local target_num = options.target_conditions.num -- 数量
    assert(target_num ~= nil)
    -- 根据位置找到对应的目标
    local target_view = {}
    if options.cell_positions and #options.cell_positions > 0 then
        for i_,cell_pos in ipairs(options.cell_positions) do
            if target_num ~= -1 and target_count >= target_num then
                break
            end
            local temp_obj_view =self:getHeroByCellPos(cell_pos)
            if temp_obj_view then
                local temp_obj= temp_obj_view:GetModel()
                if options.obj_type == temp_obj:getClassId() or options.obj_type == "all" then
                    --
                    if options.target_conditions.is_oppose and options.sender_obj:getId() ~= temp_obj:getId() and
                    options.sender_obj:getEnemyCampId() == temp_obj:getCampId() then
                    -- print("···3")
                        table.insert(options.out_target_views,temp_obj_view)
                        target_count = target_count + 1
                    end
                    --
                    if options.target_conditions.is_team and options.sender_obj:getId() ~= temp_obj:getId() and
                    options.sender_obj:getCampId() == temp_obj:getCampId() then
                    -- print("···1",options.sender_obj:getId(),temp_obj:getId())
                        table.insert(options.out_target_views,temp_obj_view)
                        target_count = target_count + 1
                    end
                    --
                    -- if options.target_conditions.is_self and
                    -- options.sender_obj:getId() == temp_obj:getId() then
                    -- -- print("···2")
                    --     table.insert(options.out_target_views,temp_obj_view)
                    --     target_count = target_count + 1
                    -- end
        ...
        local t = {}
        if options.target_conditions.is_oppose then -- 对立方
            t[#t+1]=self:getAllCampObjects(options.sender_obj:getEnemyCampId())

            -- print("···",self:getAllCampObjects(options.sender_obj:getEnemyCampId()))
            -- dump(self:getAllCampObjects(options.sender_obj:getEnemyCampId()))
        end
        if options.target_conditions.is_team then -- 队友
            t[#t+1]=self:getAllCampObjects(options.sender_obj:getCampId())
        end
        for i=1,#t do
            for k,obj_view in pairs(t[i]) do
                local obj = obj_view:GetModel()
                if obj:getId() ~= options.sender_obj:getId()
                and (options.target_conditions.obj_type == "all" or options.target_conditions.obj_type == obj:getClassId())
                then
                    if target_num ~= -1 and target_count >= target_num then
                        break
                    end
    ...
    if options.target_conditions.is_self then -- 自己
        if  target_num == -1 or target_count < target_num then
