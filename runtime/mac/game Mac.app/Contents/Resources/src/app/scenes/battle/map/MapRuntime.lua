...
function MapRuntime:tick(dt)
    ...
    -- print("···secondsDelta",secondsDelta)
    -- print("···dt",dt)
    --         handler:time(self.time_, secondsDelta)
    -- --     end
    -- end
    handler:time(self.time_, dt)

    -- 更新所有对象
    local maxZOrder = MapConstants.MAX_OBJECT_ZORDER
    for i, object in pairs(self.map_:getAllObjects()) do
        local object_=object:GetModel()
        if object_.tick then
            object_:tick(dt,self.map_)
        end

        if object_.fastUpdateView then
            object_:fastUpdateView()
        end
    end
end
function MapRuntime:onTouch(event, x, y, map)
    -- body
end
------------------------------------------------------------------------------
return MapRuntime
