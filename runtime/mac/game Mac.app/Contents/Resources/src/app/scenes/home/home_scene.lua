...
function home_scene:ctor()
   ...
   -- self.bgLayer_:align(display.LEFT_BOTTOM, 0, 0)
    ...
    local sceneSize = cc.size(display.width,display.height)
...
function home_scene:onEnter()
    ...
    self._screen_msg =require("app.scenes.DlgScreenMsg").new()
    self:addChild(self._screen_msg)


    -- local t = txml(x)
    -- dump(t)

    -- local rich=RichLabelEx:create(str3)
    -- rich:setPosition(display.cx,display.cy)
    -- self:addChild(rich)
end
----------------------------------------------------------------
...
function home_scene:test()
    ...
    cc.ui.UIPushButton.new("actor/Button01.png", {scale9 = true})
        ...
        :onButtonClicked(function()
            ...
            -- end
            -- for i=1,2 do
                G_SysMsgs:AddMsgHint(1,{Id=30100000,count=100})
                G_SysMsgs:AddMsgHint(1,{Id=30100001,count=99})
                G_SysMsgs:AddMsgHint(1,{Id=30100002,count=99})
                G_SysMsgs:AddMsgBulletin(0,{content="系统公告：服务器将于7:00～15:00进行维护！！！！！！",color_hex=""})
                G_SysMsgs:AddMsgBulletin(0,{content="系统公告：服务器将于7:00～20:00进行维护！！！！！！",color_hex=""})
            -- end
            -- for i=1,2 do
            --     local layer=cc.CSLoader:createNode("UI/DlgMsg.csb")
            --     local layer_ = layer
            --     self:addChild(layer_)
            --     -- layer_:setContentSize(cc.size(423,112))
            --     print("···",layer_:getContentSize().height)
            --     layer_:setPosition(display.cx, 0)
...
function home_scene:tick(dt)
    ...
    if self._screen_msg then
        self._screen_msg:UpdateHint(dt)
        self._screen_msg:UpdateBulletin(dt)
    end
end
----------------------------------------------------------------
-- 场景移动相关
