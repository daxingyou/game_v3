    ...
    for i=1,#paramsLst do
        ...
        local params = paramsLst[i]
         if params[SkillDefine.SkillParamL_ImpToTar_ActivateRate] ~= CommonDefine.INVALID_ID then
            ...
            if rand >= (params[SkillDefine.SkillParamL_ImpToTar_ActivateRate] or CommonDefine.RATE_LIMITE) then
        ...
        if succ then
            ...
            elseif params[SkillDefine.SkillParamL_ImpToTar_Type]==SkillDefine.AppendSkillImpType_ToCustomTar then
                ...
                local conditions = CommonUtil:ToTargetConditions(tarLogic)
                -- 目前先不制定距离
                -- conditions.atk_dis = 1
                self:getTarsByTarLogic(rMe,conditions,target_obj_views)
                --
    ...
    for i=1,#paramsLst do
        ...
        if succ then
            ...
            if params[SkillDefine.SkillParamL_ImpToTar_Type]==SkillDefine.AppendSkillImpType_ToCustomTar then
                ...
                local conditions = CommonUtil:ToTargetConditions(tarLogic)
                -- conditions.atk_dis = 1
                self:getTarsByTarLogic(rMe,conditions,targetViews)
...
function SLImpactsToTarget:conditions_check(skill_id,data)
    local paramsLst=configMgr:getConfig("skills"):GetSkillAppendImpacParams(skill_id)
    local conditions=paramsLst[1][5]
    if conditions then
        if conditions[1] == 1 then -- 条件：血量小于conditions[2]%使用
            for i,v in ipairs(data) do -- view
                if v:GetModel():getHp() < math.floor(v:GetModel():getMaxHp() * conditions[2]/100) then
                    return true
                end
            end
        elseif conditions[1] == 2 then -- 条件：rage小于
            for i,v in ipairs(data) do -- view
                if v:GetModel():getRage() < math.floor(v:GetModel():getMaxRage() * conditions[2]/100) then
                    return true
                end
            end
        end
