...
function HeroView:ctor(model,params)
    ...
    -- 图片资源
    params.img = "#"..configMgr:getConfig("hero"):GetArmArtById(model:getArmId(),"idle",model:isEnemy())
    -- 父类
...
function HeroView:init(model,params)
    ...
    ----------------------------------------------------------
    -- 显示头像
    local headIcon = configMgr:getConfig("hero"):GetHerosArt(self:GetModel():getArtId()).headIcon
    ...
    self.hpRadiusOffsetX, self.hpRadiusOffsetY = 0,10
    -- 血条
    -- self:GetModel():createView(self:GetBatch())
    self:createView(self:GetBatch())
    self:updateView()
    --
...
------------------------------------------------------------------------------
-- 更新
function HeroView:updateView(state)
    HeroView.super.updateView(self,state)
...
function HeroView:createIdleAction()
    local frameName =configMgr:getConfig("hero")
...
function HeroView:createRage(val)
    local str = (val<0 and {"rage %d"} or {"rage +%d"})[1]
    local x,y = self:getPosition()
    local label = cc.ui.UILabel.newTTFLabel_({
                            text = string.format(str,val),
                            size = 20,
                            color = display.COLOR_RED,
                        })
                        :pos(x,y)
