...
function SkillLogic:calcSkillTargets(rMe,skillId,out_target_views)
    ...
    local skillTemp = conf_skills:GetSkillTemplate(skillId)
    local skillIns = conf_skills:GetSkillInstanceBySkillId(skillId)
    local attackDis = skillIns.atkDistance
    local target_logic =  skillTemp.useTarget_type
    local target_conditions = CommonUtil:ToTargetConditions(skillTemp.target_condition,skillTemp.target_check_obj_type)
    target_conditions.atk_dis = skillIns.atkDistance
    local isChaos = rMe:isChaos()
    -- 根据目标逻辑计算攻击目标
    local out_target_views_ = {}
    self:getTarsByTarLogic(rMe,target_conditions,out_target_views_)
    if #out_target_views_<=0 then return false end

    if target_conditions.num == -1 then
        table.merge(out_target_views, out_target_views_)
        return true
    end
...
--取得目标相关
--根据目标逻辑取得目标
function SkillLogic:getTarsByTarLogic(rMe,conditions,targetViews)
    local options = {
    ...
    -- print("···",conditions.atk_dis)
    -- assert(conditions.atk_dis ~=nil)
    -- 检测攻击距离，－1时设为0，可能目标为自己
    if conditions.atk_dis == nil or conditions.atk_dis < 0 then
        conditions.atk_dis = 0
    end
    local cell_pos = rMe:getView():getCellPos()
    for i=1, #dirs do
        local offset =dirs[i]
        for j=1,conditions.atk_dis do
            local p=cc.p(cell_pos.x+offset[1]*j, cell_pos.y+offset[2]*j)
            table.insert(options.cell_positions,p)
        end
    end
    -- dump(options.cell_positions)
    -- print("···",cell_pos.x,cell_pos.y)
    -- objs = rMe:getAktDirObjsByDisAndCamp(dirs,attackDis,true)
    rMe:getMap():scanByTargetConditions(options) -- options={out_targets,cell_positions,use_target_type,is_contain_sender,sender_obj}
end
...
function SkillLogic:conditions_check(skill_id,data)
    return true
end
-----------------------------------------------------------------
return SkillLogic
