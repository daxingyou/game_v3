...
function Hero:getDefensePhysice()
    if self:GetAttrDirtyFlag(CommonDefine.RoleAttr_PhysicsDef)==true then
        ...
        local data = PLAYER:get_mgr("hero"):get_hero(self.GUID_)
        if data then
            -- 获取装备中影响人物物理防御的值
...
function Hero:getBaseDefensePhysics()
    return Hero.super.getBaseDefensePhysics(self) + self.attr_.Captain * configMgr:getConfig("skills"):GetInitDataByType(102)
end
...
function Hero:getBaseAttackPhysics()
    ...
    + self.attr_.Captain * configMgr:getConfig("skills"):GetInitDataByType(101)
end
------------------------------------------------------------------------------
-- 战法属性
...
function Hero:getAttackMagic()
    return Hero.super.getBaseAttackMagic(self) + self.attr_.Str * configMgr:getConfig("skills"):GetInitDataByType(103)
end
...
function Hero:getBaseDefenseMagic()
    return Hero.super.getBaseDefenseMagic(self) + self.attr_.Str * configMgr:getConfig("skills"):GetInitDataByType(104)
end
------------------------------------------------------------------------------
-- 计策属性
...
function Hero:getBaseAttackTactics()
    return Hero.super.getBaseAttackTactics(self) + self.attr_.Int * configMgr:getConfig("skills"):GetInitDataByType(105)
end
...
function Hero:getBaseDefenseTactics()
    return Hero.super.getBaseDefenseTactics(self) + self.attr_.Int * configMgr:getConfig("skills"):GetInitDataByType(106)
end
------------------------------------------------------------------------------
--
...
function Hero:getBaseMaxHp()
    return Hero.super.getBaseMaxHp(self) + self.attr_.Cha * configMgr:getConfig("skills"):GetInitDataByType(107)
end
------------------------------------------------------------------------------
--
function Hero:getMaxRage()
    if self:GetAttrDirtyFlag(CommonDefine.RoleAttr_MaxRage)==true then
        local value = 0
        local impact_and_skillrefix=0
        local passiveSkillPointRefix = 0
        local itemPointRefix = 0
        ...
        local baseAttr = self:getBaseMaxRage()
        -- 被动技能对属性值的影响
        passiveSkillPointRefix=self:Skill_RefixItemAttr(CommonDefine.RoleAttr_MaxRage)

        --技能对属性的影响
        impact_and_skillrefix = self:getMaxRageRefix()
        value = impact_and_skillrefix+baseAttr+itemPointRefix+itemRateRefix+passiveSkillPointRefix
        self:setIntAttr(CommonDefine.RoleAttr_MaxRage,value)
        self:ClearAttrDirtyFlag(CommonDefine.RoleAttr_MaxRage)
        self:setRage(value)
    end
    return self:getIntAttr(CommonDefine.RoleAttr_MaxRage)
end
------------------------------------------------------------------------------
-- 二级属性
