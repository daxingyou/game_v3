...
function SkillBehavior:bindMethods(object)
    ...
    local function _Impact_OnImpactActived(object,ownImpact)
        ...
        if ownImpact:getResEffectId()  then
        ...
        if ownImpact.buff_icon then
            object:getView():createBuffIcon(ownImpact:getImpactTypeId(),ownImpact.buff_icon)
        end
    end
    ...
    local function Impact_OnImpactFadeOut(object,ownImpact)
       ...
       local logic =  Impact_GetLogic(object,ownImpact)
       if ownImpact:getResEffectId() and logic:isOverTimed() then
           object:getView():removeImpactEffect(ownImpact:getImpactSpriteTagId())
       end
       if ownImpact.buff_icon and logic:isOverTimed() then
           object:getView():removeBuffIcon(ownImpact:getImpactTypeId())
       end
    ...
    local function skillsSort(object)
        local skills=object:getSkillsByUseType(SkillDefine.UseType_Auto)
    ...
    local function checkTargetConditions(object,skillId)
        local conf_skill_data = configMgr:getConfig("skills"):GetSkillData(skillId)
        local target_conditions=CommonUtil:ToTargetConditions(conf_skill_data.target_condition)
        target_conditions.atk_dis = conf_skill_data.atkDistance
        local skill_logic=object:getSkillLogicByLogicId(conf_skill_data.logicId)

        local  out_target_views = {}
        skill_logic:getTarsByTarLogic(object,target_conditions,out_target_views)
        if #out_target_views > 0 then
            if skill_logic:conditions_check(skillId,out_target_views) then
                return true
            end
        end
        return false
        --
        -- local skill_tmp=configMgr:getConfig("skills"):GetSkillTemplate(skillId)
        -- local options = {
        --     out_target_views = {},
        --     cell_positions = {},
        --     target_conditions=CommonUtil:ToTargetConditions(skill_tmp.target_condition),
        --     sender_obj = object,
        -- }
        -- object:getMap():scanByTargetConditions(options)
        -- return #options.out_target_views > 0
    end
    self:bindMethod(object,"checkTargetConditions", checkTargetConditions)
    --------------------------------------------------------------------------
    ...
    local function getSelectSkill(object)
        ...
        for k,v in pairs(object:getSkillsByUseType(SkillDefine.UseType_Auto)) do
            -- print("···",v.id)
            --判断怒气值消耗
            if not object:isBeSkill(v.id)                       -- 不为被动
                and object:getCDs():isCooldownedById(v.id)      -- 冷却结束
                and object:checkCondition(v.id)                 -- 消耗条件
                and not object:isInSceneSkill(v.id)
                and object:checkTargetConditions(v.id)
            then
    ...
    local function addSkill(object,skill)
        ...
        -- if skill.useType==SkillDefine.UseType_Auto then
            ...
            table.insert(object.skillsByUseType_[skill.useType],skill)
        -- else
    ...
    local function getHeroSkillsOfSkillRule(object)
        ...
        --     return s
        -- end
        -- s=configMgr:getConfig("hero"):getHeroSkills(skillRuleOfHero)
    ...
    local function getArmSkillsOfSkillRule(object)
        local armData = configMgr:getConfig("hero"):GetArmsData(object:getArmId())
        assert(armData~=nil,"getArmSkillsOfSkillRule():arm nil")
        local skillRuleOfArm = armData.skillRule
        local s=configMgr:getConfig("hero"):getArmSkills(skillRuleOfArm)
