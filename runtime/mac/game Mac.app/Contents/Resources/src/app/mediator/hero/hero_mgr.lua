...
----------------------------------------
-- 根据guid得到hero
function hero_mgr:get_info(dataId)
...
function hero_mgr:get_hero(dataId)
...
function hero_mgr:update(newdata)
    ...
    -- 添加一条新数据
    self.__data[newdata.dataId] = hero.new(newdata)

    -- 刷新相关
    self.__data[newdata.dataId]:flush_item_effect()
    -- print("---------update")
    -- dump(self.__data)

end
----------------------------------------
function hero_mgr:remove( dataId )
