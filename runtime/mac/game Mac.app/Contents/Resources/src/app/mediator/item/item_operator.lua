...
function item_operator:get_item(dataId)
	return self:get_mgr(dataId):get_info(dataId)
end

function item_operator:update(info)
	if info.dataId > 0 then
		local mgr = self:get_mgr(info.dataId)
		if mgr then
			if info.num == 0 then
				return mgr:remove(info.dataId)
			else
...
function item_operator:get_conf_info(dataId)
	local kind = item_helper.get_serial_class(dataId)
	return configMgr:getConfig(kind):get_info(dataId)
end
...
function item_operator:get_num_by_dataId(dataId)
	...
	for k,v in pairs(datas) do
		if v:get_info().dataId == dataId then
...
function item_operator:compound( hero_dataId, item_dataId)
	...
	if info.elevel >= 0 then
		...
		t = player:get_mgr("hero"):get_equip(hero_dataId,t)
	...
	for k,v in ipairs(info.stuff_id) do
		-- print(k,v)
		if v > 0 then
			local item = self:get_mgr(v):get_info(v)
...
function item_operator:enhance( hero_dataId, equip_dataId )
	...
	local equip_info = player:get_mgr("hero"):get_equip(hero_dataId,equip_dataId)
	...
	return player:send("CS_EquipEnhance",{hero_dataId = hero_dataId, equip_dataId = equip_dataId})
end
