...
function UIListView:updataScrollArea(callback_)
    ...
    self.scrollAreaSize_=cc.size(sizeItem.width*(self.amountCol_),sizeItem.height*(amountRow__))
...
function UIListView:InitialItem()
    ...
    self.lstCtrlItem:setVisible(false)
end
function UIListView:SetItemTemp(tempItem)
    self.lstCtrlItem = tempItem
end
function UIListView:ClearItem(is_clear)
    for i=1,self:getCount() do
        self:DelItem(i,is_clear)
    end
end
function UIListView:DelItem(index,is_clear,is_updata)
