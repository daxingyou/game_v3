...
function M:init( params )
    ...
    if self.is_no_modle then
        ...
        -- self:setTouchLayerEnabled(true,true)
        -- 模态吞噬触摸
        self:setTouchSwallowEnabled(true)
        self._root_widget:setTouchEnabled(true)
        self._root_widget:setTouchSwallowEnabled(false)
    end
end
------------------------------------------------------------------------------
...
function M:ListenClose()
    if self._root_widget then
        self._root_widget:addTouchEventListener(function(sender, eventType)
            -- print("···1",self.__cname)
                                    local ccs = self.ccs
...
function M:setWidgetOptions( widget, options)
    ...
    if options.pos then
       widget:setPosition(options.pos.x,options.pos.y)
    end
