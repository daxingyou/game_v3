...
function UIHeroInfo:init( params )
    self.heroinfo = PLAYER:get_mgr("hero"):get_info(params.params.GUID)
...
function UIHeroInfo:UpdataHero(options)
    ...
    w:setString(self.heroinfo.nickname)
    --国家
    w=UIHelper:seekWidgetByName(root, "country")
    w:setString(self.heroinfo.countryInfo.name)
    --头像
        ...
        img:addTouchEventListener(function(sender, eventType)
                                    ...
                                    if eventType == ccs.TouchEventType.ended then
                                        self:getUIManager():openUI({uiScript=require("app.ui.UIHeroSkillUp"),
                                            ccsFileName="UI/hero_skillup.json",open_close_effect=true,
                                                params={skillId=skillId}})
                                    end
                                end)
        lab:setString(skillTemp.nickname)
        labLev:setString(string.format("lv.%d",skillIns.lev))
...
function UIHeroInfo:UpdataHeroArr(options)
    if options.con then
        ...
        local rich=RichLabelEx:create(options.con,ctrl,
        {is_ignore_adapt_size=false})
        -- ctrl:setString(options.con)
    end
end
----------------------------------------------------------------------------------------
--兵种界面相关
...
function UIHeroInfo:ListenEquip()
    ...
    for i=1,6 do
        ...
        slot:addTouchEventListener(function(sender, eventType)
                                    if eventType == self.ccs.TouchEventType.ended then
                                            self:openUI({
                                                        uiScript=require("app.ui.UIEquipUse"),open_close_effect=true,
    ...
    btn:addTouchEventListener(function(sender, eventType)
                                    if eventType == self.ccs.TouchEventType.ended then
                                            self:openUI({
                                                        uiScript=require("app.ui.UIEquipMake"),open_close_effect=true,
...
function UIHeroInfo:UpdataHeroEquip()
    ...
    -- 已装备的道具
    self.heroinfo = PLAYER:get_mgr("hero"):get_info(self.heroinfo.GUID)
    ...
    for k,v in ipairs(self._equip_solts ) do
        ...
        if v.state == 0 or v.state == 1 then
            if equip_info then -- 已有装备
               ...
               --          texture = equip_info.icon,
               --          pos     = cc.p(v:getPositionX(),v:getPositionY()),
               --  }):addTo(v):setColor(cc.c3b(299, 587, 114))
               --

                CommonUtil:NewGraySprite(equip_info.icon)
