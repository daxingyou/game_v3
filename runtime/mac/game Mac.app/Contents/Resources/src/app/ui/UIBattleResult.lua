...
function UIBattleResult:UpdataData()
    ...
    starTemp:setVisible(false)

    if self._args.stars > 0 then -- 胜利
        result:setString("战斗胜利")
        for i=1,3 do
            local star_ = nil
            if i == 1 then
                star_ = starTemp
            ...
            if self._args.stars < i then
                star_:getChildByName("ImageStar"):setVisible(false)
        ...
        lev:setString(string.format("Lev:%d Exp:+%d",PLAYER:get_basedata().level,conf_stage.a_exp))
        --
        self:getWidgetByName("LabelVal1"):setString("+"..conf_stage.a_money)
        -- 物品
        ...
        for i=1,#self._args.award do
            ...
            self:getWidgetByName_(item,"LabelName"):setString(conf_data:get_info(self._args.award[i].dataId).name)
            self:getWidgetByName_(item,"Item"):loadTexture(icon)
           --self._args.award[i].num
        end

    else -- 失败
        result:setString("战斗失败")
    end
