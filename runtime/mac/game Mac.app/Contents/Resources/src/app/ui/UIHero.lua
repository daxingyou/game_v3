...
function M:init( params )
    ...
    local heros = PLAYER:get_mgr("hero"):get_data()
...
function M:addItem(heroDt)
    ...
    w:addTouchEventListener(function(sender, eventType)
            ...
            elseif eventType == ccs.TouchEventType.moved then
                ...
                if self.drag.isClickDown and self.drag.isMove then
                    ...
                    -- 删除
                    if self.newCell then
                        self.newCell:removeFromParent(true)
                        self.newCell = nil
                    end
                end
                --点击弹起处理
                if self.drag.isClickDown and not self.drag.isMove then
                    self:getUIManager():openUI({uiScript=require("app.ui.UIHeroInfo"),ccsFileName="UI/hero_info.json",
                                                params={GUID=heroDt.GUID},open_close_effect=false})
                end
            end
        end)
    ...
    w:setString(string.format("等级:%d 战斗力:%d",heroinfo.level,heroinfo.attr.PhysicsAtk+heroinfo.attr.PhysicsDef))
...
function M:addReadyHero(data)
    ...
    if not data.bNoUpdateView then
        ...
        if data.slot then
            local heroinfo = PLAYER:get_mgr("hero"):get_info(data.heroDt.GUID)
            if heroinfo then
...
function M:createFormationItems()
    self:getWidgetByName("PanelScrollView",function ( PanelScrollView )
                        local ScrollView = PanelScrollView:getChildByName("ScrollView")
                        local tplItemFmt = ScrollView:getChildByName("PanelItem")
...
function M:addItemFormation(index,tplItem)
    ...
    --名称
    local w = UIHelper:seekWidgetByName(widgetItem, "name")
    w:setString(string.format(StringData[100000003],StringData[100000004],1) )
    --人数
    w = UIHelper:seekWidgetByName(widgetItem, "heroInfo")
    w:setString(string.format(StringData[100000001],"5") )
    --阵型信息
...
function M:setSelectedFormationCell(fId,index,isLight)
    ...
    if isLight then
        wgfmt.arrCell[index]:setString(tostring(index))
    else
        wgfmt.arrCell[index]:setString("-")
    end
end
--槽索引转为阵型位置
