...
function UIEquipMake:init( params )
    --self.heroinfo = PLAYER:get_mgr_heros():get_info(params.GUID)
    ...
    for i=1,6 do
        ...
        UIHelper:seekWidgetByName(item,"ItemFrameLight"):setVisible(false)
        ...
        item_frame:addTouchEventListener(function(sender, eventType)
                                        ...
                                        if eventType == self.ccs.TouchEventType.ended then
                                            ...
                                            if self._current_info.select_frame then
                                                self._current_info.select_frame:setVisible(false)
                                            end
                                            if self._equip_slots[i].use then
                                                            print("···9")
                                                item_frame_light:setVisible(true)
...
function UIEquipMake:ShowDlg(index)
    if self._current_info.select_dlg then
        self._current_info.select_dlg:setVisible(false)
    end
    -- for i=1,3 do
...
function UIEquipMake:UpdataDlg(index,equip_guid,options)
    -- equip info
    local equip_info = PLAYER:get_mgr("hero"):get_equip_by_guid(self._hero_info.GUID, equip_guid)
    local equip_info_ = item_operator:get_equip_info( equip_info )
    -- dlg
    ...
    if index == 1 then
        ...
        if conf_enhance_next then
            ...
            w:setString(string.format(StringData[100000043],equip_info_.elevel,arrt_name_text.."+"..current_).."\n"
            ...
            w:setString(string.format(StringData[890000003],num,conf_enhance_next.stuff_num))
            if num < conf_enhance_next.stuff_num then
                w:setColor(display.COLOR_RED)
            end
            --
            w = UIHelper:seekWidgetByName(dlg,"GoldNum")
            w:setString(conf_enhance_next.money)
...
function UIEquipMake:_GetDlg(index)
    ...
    -- initial
    if file_name then
        dlg=GUIReader:shareReader():widgetFromJsonFile(file_name)
